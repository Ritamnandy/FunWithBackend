...
const changeCurrentPassword = asyncHandler(async (req, res) => {
    ...
    if (!isPasswordCurrect) {
        return res.status(400)
            .json(new ApiError(400, "password is incurrect", ["password is incurrect"]))
    }
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
    ...
    if (!username?.trim()) {
        return res.status(400).json(new ApiError(400, "username is missing", ["username is missing"]))
    }
    const channel = await User.aggregate([
        ...
        {
            $project: { fullname: 1, username: 1, subscribersCount: 1, channelsubscriberedCount: 1, avatar: 1, coverImage: 1, email: 1 }
        }
    ])
    ...
    if (!channel?.length) {
        res.status(404).json(new ApiError(404, "channel dos\'t exists", ["channel dos\'t exists"]))
    }
...
const userVideoHistory = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, "watch histry", user[0].watchHistory))
